...
const Halls = () => {
  ...
  const handleSave = async () => {
    if (!form.name) return
    try {
      const payload = { ...form, capacity: Number(form.rows) * Number(form.cols) }
      if (modalMode === 'add') await addHall(payload)
      else await updateHall({ ...selected, ...payload })
      closeModal()
    } catch { /* error */ }
  }
    ...
    <div>
      ...
      <div className="bg-cinema-card border border-cinema-border rounded-lg overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {halls.map((h, i) => (
              <tr key={h.id} className="hover:bg-cinema-card2/40 transition-colors">
                <td className="px-4 py-3 text-sm text-white border-t border-cinema-border/50 text-cinema-muted font-mono text-xs">{i + 1}</td>
                <td className="px-4 py-3 text-sm text-white border-t border-cinema-border/50">
                  <div className="flex items-center gap-2">
                    <Building2 size={14} className="text-cinema-red" />
                    <span className="font-semibold">{h.name}</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-sm text-white border-t border-cinema-border/50">
                  <div className="flex items-center gap-1 text-sm">
                    <Users size={12} className="text-cinema-muted" />
                    <span className="font-mono text-white">{h.capacity}</span>
                    <span className="text-cinema-muted text-xs">seats</span>
                  </div>
                </td>
                <td className="px-4 py-3 text-sm text-white border-t border-cinema-border/50 text-cinema-muted text-xs font-mono">
      ...
      <Modal isOpen={!!modalMode} onClose={closeModal} title={modalMode === 'add' ? 'Add Hall' : 'Edit Hall'}>
        <div className="space-y-4">
          ...
          <div className="bg-cinema-card2 border border-cinema-border rounded-lg px-3 py-2 text-xs text-cinema-muted">
