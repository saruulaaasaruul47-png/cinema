...
const sumRows = async (connection, tableName, column, where = "1 = 1") => {
  ...
  const [rows] = await connection.execute(
    `SELECT COALESCE(SUM(${column}), 0) AS total FROM ${tableName} WHERE ${where}`
  );
...
const getDashboardAnalytics = async () => {
  ...
  try {
    ...
    const [seatRows] = await connection
      .execute(`SELECT COALESCE(SUM(seat_count), 0) AS totalSeats FROM cinema_halls`)
      .catch(() => [[{ totalSeats: 0 }]]);

    const bookedSeats = await sumRows(
      connection,
      "bookings",
      "JSON_LENGTH(seats)",
      "status = 'confirmed'"
    ...
    const [recentBookings] = await connection
      .execute(
    ...
    const [revenueByDay] = await connection
      .execute(
